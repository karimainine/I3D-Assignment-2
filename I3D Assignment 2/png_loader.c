...
Image* load_png(const char* filename)
{
	...
	while (reading == 1 && ferror(file) == 0) /* break on finish or an error */
	{
		...
		if (str_compare(chunkType, "IHDR") == 0) /* png header chunk */
		{
			...
			if (interlaceMethod != 0) {printf("Error: Loader does not support interlacing\n"); inflateEnd(&strm); fclose(file); return NULL;}
		...
		if (read != chunkSize)
		{printf("Error: Unknown error while parsing chunk %s. %i unread.\n", chunkType, chunkSize - read); free(data); inflateEnd(&strm); fclose(file); return NULL;}
			...
			for (k = 0; k < bytesPerPixel; ++k)
			{
				...
				switch (scanFilter)
				{
					...
					case 2: /* Up */
						...
						break;
					case 3: /* Average */
						data[((height-i-1) * width + j) * bytesPerPixel + k] = cbyte + (unsigned char)(((int)pback + (int)pup) / 2);
						break;
					case 4: /* Paeth */
